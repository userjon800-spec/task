  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (!companyData.company || companyData.company.length === 0) {
          setError("not_found");
          setLoading(false);
          return;
        }

        setCompany(companyData.company[0]);

        if (companyData.sorted && companyData.sorted.length > 0) {
          setTopRepos(companyData.sorted.slice(0, 5));
        } else {
          setEmptyRepos(true);
        }
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
            {/* PieChart - sizning komponentingiz */}
            <div className="lg:col-span-1">
              {/* <PieChart data={languages.language} /> */}
            </div>
